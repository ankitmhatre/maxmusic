		...
		public Bitmap createBitmap(Song song, long maxPxCount) {
			...
			try {
				...
				if (inputStream == null && (CoverCache.mCoverLoadMode & CoverCache.COVER_MODE_SHADOW) != 0) {
					String shadowPath = "/sdcard/Music/.maxmusic/"+(song.artist.replaceAll("/", "_"))+"/"+(song.album.replaceAll("/", "_"))+".jpg";
				...
				// no cover art found
				Log.v("MaxMusic", "Loading coverart for "+song+" failed with exception "+e);
			}
			// failed!
			return null;
		}
